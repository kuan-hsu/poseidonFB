...
class CCustomCompilerOptionDialog : CBaseDialog
{
	private:
	import				tools;
	Ihandle*			listTools;
	...
	void createLayout()
	{
		...
		{
			CCustomCompilerOptionDialog.tempCustomCompilerOptions ~= GLOBAL.customCompilerOptions[i].dup;
		...
		Ihandle* labelOptions = IupLabel( toStringz( GLOBAL.languageItems["prjopts"].toDString ~ ":" ) );
		IupSetAttributes( labelOptions, "SIZE=54x16" );
		...
		Ihandle* hBoxOption = IupHbox( labelOptions, textOptions, null );
		IupSetAttribute( hBoxOption, "ALIGNMENT", "ACENTER" );

		
		Ihandle* labelSEPARATOR = IupLabel( null ); 
		IupSetAttribute( labelSEPARATOR, "SEPARATOR", "HORIZONTAL");
		Ihandle* vBoxLayout = IupVbox( frameList, hBoxOption, labelSEPARATOR, bottom, null );
	...
	this( int w, int h, char[] title, bool bResize = false, char[] parent = "MAIN_DIALOG" )
	{
		...
		IupSetAttribute( btnCANCEL, "TITLE", GLOBAL.languageItems["close"].toCString );
		IupSetAttribute( btnOK, "TITLE", GLOBAL.languageItems["apply"].toCString );
		IupSetCallback( btnOK, "ACTION", cast(Icallback) &CCustomCompilerOptionDialog_btnOK );
	}
...
extern(C) // Callback for CFindInFilesDialog
{
	...
	{
		if( index < CCustomCompilerOptionDialog.tempCustomCompilerOptions.length )
		{
			char[] s = CCustomCompilerOptionDialog.tempCustomCompilerOptions[index];
			int pos = Util.rindex( s, " %::% " );
			if( pos < s.length )
			{
				Option	= Util.trim( s[0..pos] );
				Name	= Util.trim( s[pos+6..$] );
			}
		}
	}
	
	private int CCustomCompilerOptionDialog_btnOK( Ihandle* ih )
	{
		...
		if( textHandle != null )
		{
			if( id > 0 ) CCustomCompilerOptionDialog.tempCustomCompilerOptions[id-1] = fromStringz( IupGetAttribute( textHandle, "VALUE" ) ).dup ~ " %::% " ~ fromStringz( IupGetAttributeId( toolsHandle, "", id ) ).dup;

			GLOBAL.customCompilerOptions.length = 0;
			foreach( char[] s; CCustomCompilerOptionDialog.tempCustomCompilerOptions )
			{
	...
	private int CCustomCompilerOptionDialog_btnToolsAdd( Ihandle* ih ) 
	{
		...
		if( newFileName.length )
		{
			...
			CCustomCompilerOptionDialog.tempCustomCompilerOptions[$-1] = " %::% " ~ newFileName;
	...
	private int CCustomCompilerOptionDialog_btnToolsErase( Ihandle* ih ) 
	{
		...
		for( int i = index - 1; i < CCustomCompilerOptionDialog.tempCustomCompilerOptions.length - 1; ++ i )
		{
			CCustomCompilerOptionDialog.tempCustomCompilerOptions[i] = CCustomCompilerOptionDialog.tempCustomCompilerOptions[i+1];
		}
	...
	private int CCustomCompilerOptionDialog_btnToolsUp( Ihandle* ih ) 
	{
		...
		if( itemNumber > 1 )
		{
			...
			char[] temp = CCustomCompilerOptionDialog.tempCustomCompilerOptions[itemNumber-1];
			CCustomCompilerOptionDialog.tempCustomCompilerOptions[itemNumber-1] = CCustomCompilerOptionDialog.tempCustomCompilerOptions[itemNumber];
			CCustomCompilerOptionDialog.tempCustomCompilerOptions[itemNumber] = temp;
		}
