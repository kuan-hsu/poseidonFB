...
version(DLL)
{
	...
	CASTnode createNodeFromJSON( char[] document )
	{
		...
		foreach( char[] s; Util.splitLines( document.dup ) ) // The document(Json Txt) is from DLL, so dup it
		{
			...
			int colonPos = Util.index( s, "\": " );
			if( colonPos < s.length )
			{
				left = s[1..colonPos];	// remove first "
				if( s[$-1] == '"' )
					right = s[colonPos+4..$-1]; // remove "
				else if( s[$-1] == ',' )
					right = s[colonPos+4..$-2]; // remove "
				else 
					right = s[colonPos+3..$];;
			...
			else
			{
				if( right.length )
				{
					...
					{
						case "line":
							member.lineNumber	= Integer.atoi( right ); break;
							
						case "tail":
							member.endLineNum	= Integer.atoi( right ); break;
							
						case "name":
							member.name		= right; break;
							
						case "kind":
							member.kind		= Integer.atoi( right ); break;
							
						case "prot":
							member.protection	= right; break;
							
						case "type":
							member.type		= right; break;
							
						case "base":
							member.base		= right; break;
							
						case "sons":
							if( activeNode !is null )
							{
								auto newNode = new CASTnode( member.name, member.kind, member.protection, member.type, member.base, member.lineNumber, member.endLineNum );
								activeNode.addChild( newNode );
								activeNode = newNode;
							}
							else
							{
								activeNode = new CASTnode( member.name, member.kind, member.protection, member.type, member.base, member.lineNumber, member.endLineNum );
							}
							break;
				...
				else
				{
					if( left.length )
					{
						if( left[0] == '}' ) 
						{
							if( activeNode.getFather !is null )	activeNode = activeNode.getFather; else	return activeNode;
						}
					}
				}
			}
